...
`DO-NOT-REIMPORT.csv` lists every API entry deliberately removed from this
directory, with the class of removal, the evidence trail, and the exact
condition under which it may be reimported.
...
An entry leaves this list only by meeting its `reimport_condition`, recorded
